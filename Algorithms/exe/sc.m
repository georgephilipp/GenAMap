...
%
%   Output : predict_labels : N-by-1 vector containing predicted labels
%            evd_time       : running time for eigendecomposition
%            kmeans_time    : running time for k-means
%            total_time     : total running time
%
%   Authors: Wen-Yen Chen, Yangqiu Song, Hongjie Bai, Chih-Jen Lin, 2008.
%
...
if (sigma == 0) % Self-tune spectral clustering
...
OPTS.disp = 0;
%val = eigs(L,3200, 'SM', OPTS);
%filename = ['eigvals' num2str(num_clusters) '.txt'];
%save(filename, 'val', '-ascii');
[V, val] = eigs(L, num_clusters, 'SM', OPTS);
time2 = toc;
