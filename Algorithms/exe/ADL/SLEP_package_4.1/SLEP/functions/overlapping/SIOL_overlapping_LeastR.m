function [x, funVal, ValueL, res]=SIOL_overlapping_LeastR(A, y, z, opts)
% changed such that input and output matrix are A and y
%
%%
...
%                         a sparse matrix
%                         or a DCT matrix
%             seunghak: in fact it should be mT x nT for SIOL
...
if ~isfield(opts,'T')
    error('need to specify the number of responses T');
end
T = opts.T; % total number of responses;
...
for t=1:T
    ...
    ATy((t-1)*n+1:t*n)=A'*y(start_pos:end_pos);
    start_pos=end_pos+1;
end
...
%if (opts.nFlag==0)
    %% Seunghak [modified]
    %Ax=A* x;
    Ax=zeros(m*T,1); % x: n*T by 1 vector. A m*T by n*T matrix
    start_pos=1;
    for t=1:T
        end_pos=start_pos+n-1;
        Ax((t-1)*m+1:t*m)=A*x(start_pos:end_pos);
        start_pos=end_pos+1;
    end
...
if (opts.mFlag==0 && opts.lFlag==0)    
    ...
    for iterStep=1:opts.maxIter
        ...
        beta=(alphap-1)/alpha;    
        s=x + beta* xxp; % nT by 1
        
        % --------------------------- step 2 ---------------------------
        % line search for L and compute the new approximate solution x
        
        % compute the gradient (g) at s
        As=Ax + beta* (Ax-Axp); % mT by 1
        
        % compute AT As
        %if (opts.nFlag==0)
           %% Seunghak [modified]
            %ATAs=A'*As; % nT by mT * (mT by 1)
            ATAs=zeros(n*T,1); 
            start_pos=1;
            for t=1:T
                end_pos=start_pos+m-1;
                ATAs((t-1)*n+1:t*n)=A'*As(start_pos:end_pos);
                start_pos=end_pos+1;
            end
        ...
        while (1)
            ...
            [x,gap,penalty2]=overlapping(v, T*n, groupNum, lambda1/L, lambda2/L,...
