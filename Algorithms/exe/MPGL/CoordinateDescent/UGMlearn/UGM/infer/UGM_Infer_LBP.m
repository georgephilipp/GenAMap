function [nodeBel, edgeBel, logZ] = UGM_Infer_LBP(nodePot,edgePot,edgeStruct,infoStruct)

if infoStruct.useMex
    [nodeBel,edgeBel,logZ] = UGM_Infer_LBPC(nodePot,edgePot,int32(edgeStruct.edgeEnds),int32(infoStruct.nStates),int32(edgeStruct.V),int32(edgeStruct.E),infoStruct.maxIter);
else
    [nodeBel, edgeBel, logZ] = Infer_LBP(nodePot,edgePot,edgeStruct,infoStruct);
end

end

function [nodeBel, edgeBel, logZ] = Infer_LBP(nodePot,edgePot,edgeStruct,infoStruct)
...
maximize = 0;
new_msg = UGM_LoopyBP(nodePot,edgePot,infoStruct,edgeStruct,maximize);
