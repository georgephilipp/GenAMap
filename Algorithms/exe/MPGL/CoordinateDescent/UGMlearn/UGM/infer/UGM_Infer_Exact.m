function  [nodeBel, edgeBel, logZ] = UGM_Infer_Exact(nodePot, edgePot, edgeStruct, infoStruct)
% INPUT
% nodePot(node,class)
...
assert(prod(infoStruct.nStates) < 50000000,'Brute Force Exact Inference not recommended for models with > 50 000 000 states');

if infoStruct.useMex
   [nodeBel,edgeBel,logZ] = UGM_Infer_ExactC(nodePot,edgePot,int32(edgeStruct.edgeEnds),int32(infoStruct.nStates));
else
   [nodeBel,edgeBel,logZ] = Infer_Exact(nodePot,edgePot,edgeStruct,infoStruct);
end
end

function  [nodeBel, edgeBel, logZ] = Infer_Exact(nodePot, edgePot, edgeStruct, infoStruct)
