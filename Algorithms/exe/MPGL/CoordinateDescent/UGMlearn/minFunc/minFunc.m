...
%   - 'tensor': Tensor
%       (uses user-supplied Hessian matrix and numerically computes
%           Tensor of 3rd partial derivatives)
%
...
[verbose,verboseI,debug,doPlot,maxFunEvals,maxIter,tolFun,tolX,method,...
    ...
    DerivativeCheck,Damped,HvFunc,bbType,cycle] = minFunc_processInputOptions(options);
...
if nargout(funObj) == 1
    fprintf('Warning: User-supplied objective function returns only 1 value, setting numDiff to 1\n');
    numDiff = 1;
end
