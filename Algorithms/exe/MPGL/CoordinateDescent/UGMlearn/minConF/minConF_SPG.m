function [x,f,funEvals] = minConf_SPG(funObj,x,funProj,options)
% function [x,f] = minConF_SPG(funObj,funProj,x,options)
%
...
%   @funProj(x): function that returns projection from x onto C
%
%   options:
...
%       differentials)
%       armijoParam: sufficient decrease parameter in Armijo condition
...
[verbose,numDiff,optTol,maxIter,suffDec,interp,memory,useSpectral,curvilinear,feasibleInit,testOpt,bbType] = ...
    myProcessOptions(...
    options,'verbose',1,'numDiff',0,'optTol',1e-7,'maxIter',25,'suffDec',1e-4,...
...
while funEvals <= maxIter
    ...
    % Protect against a ridiculously big step
