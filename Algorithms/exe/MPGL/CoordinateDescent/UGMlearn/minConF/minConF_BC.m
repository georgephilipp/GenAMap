...
[verbose,numDiff,optTol,maxIter,suffDec,init,interp,method,corrections] = ...
    myProcessOptions(...
    options,'verbose',3,'numDiff',0,'optTol',1e-7,'maxIter',250,'suffDec',1e-4,...
    'init',0,'interp',1,'method','lbfgs','corrections',100);
...
while funEvals <= maxIter
    ...
    if ~secondOrder
        t = min(t,1e4/(1+sum(abs(g(working)))));
    end

    % Evaluate the Objective and Projected Gradient at the Initial Step
    if secondOrder
        [f_new,g_new,H] = funObj(projectBounds(x + t*d,LB,UB));
    else
        [f_new,g_new] = funObj(projectBounds(x + t*d,LB,UB));
    end
    funEvals = funEvals+1;
