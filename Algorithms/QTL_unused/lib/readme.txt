    ...
    B. Set the path by doing one of the following:
        ...
        * Add the MCR directory to the environment variable by issuing 
        ...
        NOTE: When deploying C and C++ standalone applications, it is 
              possible to run the shell script file run_libzscore.sh 
...
3. Collect the following files, based on what you are deploying: 
    ...
    All Platforms:
        * libzscore.ctf (If present)(Component Technology File archive,
          platform-dependent on target machine)

    Windows:
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          installer)
        * libzscore.exe (application created by MATLAB Compiler)

    UNIX:
        * MCRInstaller.bin (MATLAB Compiler Runtime installer)
        * libzscore (application created by MATLAB Compiler)
        * run_libzscore.sh (shell script that can be run to 
    ...
    All Platforms:
        * libzscore.so (shared library, file extension varies by 
          platform)
        * libzscore.h (library header file)
        * libzscore.ctf (If present)(Component Technology File archive,
    ...
    Windows:
        * libzscore.ctf (Component Technology File archive, contains 
          compiled M-code)    
        * libzscore.xml (documentation files)
        * libzscore.pdb (program database file - if DEBUG option 
          selected)
        * libzscore.dll (component assembly file)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
    ...
    Windows:
        * libzscore.ctf (Component Technology File archive, contains 
          compiled M-code)
        * libzscore_1_0.dll (COM component)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)      
    ...
    All Platforms: 
        * libzscore.jar (Java class files and compiled M-code)
    ...
    Windows:
        ...
        * libzscore.ctf (Component Technology File archive, contains 
          compiled M-code)
        * libzscore.xla (Optionally created and placed in
