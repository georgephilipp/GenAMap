    ...
    B. Set the path by doing one of the following:
        ...
        * Add the MCR directory to the environment variable by issuing 
        ...
        NOTE: When deploying C and C++ standalone applications, it is 
              possible to run the shell script file run_libcalcpval.sh 
...
3. Collect the following files, based on what you are deploying: 
    ...
    All Platforms:
        * libcalcpval.ctf (If present)(Component Technology File archive,
          platform-dependent on target machine)

    Windows:
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          installer)
        * libcalcpval.exe (application created by MATLAB Compiler)

    UNIX:
        * MCRInstaller.bin (MATLAB Compiler Runtime installer)
        * libcalcpval (application created by MATLAB Compiler)
        * run_libcalcpval.sh (shell script that can be run to 
    ...
    All Platforms:
        * libcalcpval.so (shared library, file extension varies by 
          platform)
        * libcalcpval.h (library header file)
        * libcalcpval.ctf (If present)(Component Technology File archive,
    ...
    Windows:
        * libcalcpval.ctf (Component Technology File archive, contains 
          compiled M-code)    
        * libcalcpval.xml (documentation files)
        * libcalcpval.pdb (program database file - if DEBUG option 
          selected)
        * libcalcpval.dll (component assembly file)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
    ...
    Windows:
        * libcalcpval.ctf (Component Technology File archive, contains 
          compiled M-code)
        * libcalcpval_1_0.dll (COM component)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)      
    ...
    All Platforms: 
        * libcalcpval.jar (Java class files and compiled M-code)
    ...
    Windows:
        ...
        * libcalcpval.ctf (Component Technology File archive, contains 
          compiled M-code)
        * libcalcpval.xla (Optionally created and placed in
