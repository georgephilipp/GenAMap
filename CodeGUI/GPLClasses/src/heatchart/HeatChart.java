...
public class HeatChart {
	...
	private double colourScale;
        private double dataMin=-5;
        private double dataMax=-5;
	...
	public HeatChart(double[][] zValues) {
        ...
        public void setDataMin(double dmin)
        {
            this.dataMin = dmin;
        }
        
        public void setDataMax(double dmax)
        {
            this.dataMax = dmax;
        }
...
//	 */
	public void setChartWidth(int width) {
		this.chartWidth = width;
                //this.cellWidth = (double)hmwidth / (double)this.zValues[0].length ;
	}
...
//	 */
	public void setChartHeight(int height) {
		...
		this.cellHeight = (double)height / (double)this.zValues.length;
	}
	...
	private void measureComponents() {
		...
		if (xValuesHorizontal) {
			...
			xValuesVerticalSize = xAxisValuesWidthMax;
                ...
                cellWidth = (chartWidth -  ((2 * chartMargin) + yAxisLabelHeight + yValuesHorizontalSize + axisThickness))/(double)zValues[0].length;
		cellHeight = (chartHeight -  ((2 * chartMargin) + xAxisLabelHeight + xValuesVerticalSize + titleHeight + axisThickness))/(double)zValues.length;

		// Calculate heatmap dimensions.
		heatMapWidth = (int)(zValues[0].length * cellWidth);
		heatMapHeight = (int)(zValues.length * cellHeight);
	...
	private void drawHeatMap(Graphics2D chartGraphics, double[][] data) {
		...
		double dataMin = this.dataMin;//min(data);
		double dataMax = this.dataMax;//max(data);
		...
		for (int x=0; x<noXCells; x++) {
                        int wX = (int)Math.floor(cellWidth);
                        if(isIncreaseWidthVal(x, locX, cellWidth))
                        {
                            wX = wX + 1;
                        }
			for (int y=0; y<noYCells; y++) {
				...
				//double cellY = y*(cellHeight);
                                ...
                                int hY = (int) Math.floor(cellHeight);
                                if(isIncreaseWidthVal(y, locY, cellHeight))
                                {
                                    hY = hY + 1;
                                }
				
				heatMapGraphics.fillRect(locX, locY, wX, hY);
        ...
        int[] order = {};
        private boolean isIncreaseWidthVal(int idx, int curLoc, double width)
        {
            double loc = idx * width;
	...
	private void drawXValues(Graphics2D chartGraphics) {
		...
		for (int i=0; i<xValues.length; i++) {
			...
			if (xValuesHorizontal) {
				...
				int valueXPos = (int)((i * cellWidth) + ((cellWidth / 2) - (valueWidth / 2)));
				...
				chartGraphics.drawString(xValueStr, valueXPos, valueYPos);
                                ...
                                //int cW = (int) cellWidth;
				int valueXPos = (int)(chartMargin + yAxisLabelHeight + axisThickness + (yValuesHorizontal ? yAxisValuesWidthMax : yAxisValuesHeight) + (i * cellWidth) + ((cellWidth / 2) + (xAxisValuesHeight / 2)));
	...
	private void drawYValues(Graphics2D chartGraphics) {
		...
		for (int i=0; i<yValues.length; i++) {
			...
			if (yValuesHorizontal) {
				...
				int valueYPos = (int)(chartMargin + titleHeight + (i * cellHeight) + (cellHeight/2) + (yAxisValuesAscent/2));
				
				chartGraphics.drawString(yValueStr, valueXPos, valueYPos);
			} else {
				int valueXPos = chartMargin + yAxisLabelHeight + yAxisValuesAscent;
                                //int cH = (int) cellHeight;
				int valueYPos = (int)(chartMargin + titleHeight + (i * cellHeight) + (cellHeight/2) + (valueWidth/2));
